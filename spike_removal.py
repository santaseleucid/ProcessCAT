...
def spike_removal(rough_passed, dist_passed, curvature=-1e7, edge_slope=5e3, peak_factor=3.0):
    ...
    d1 = np.concatenate(([0], np.sign(drdx[start-1:stop-1]), [0]))  # get sign of value to the left and right of every point n
    d2 = np.concatenate(([0], np.sign(drdx[start+1:stop+1]), [0]))

    inflections = np.logical_not(np.equal(d1,  d2))  # look for locations where the sign on the left and right change

    y = d2rdx2 < curvature  # look for locations where 2nd derivative is negative enough to warrant a spike

    target_points = np.flatnonzero(np.logical_and(inflections, y)) #list of points that fit the above criteria

    detected_spike = True  # secondary stopping condition
    num_spikes = 0  # sanity check variable - can be removed

    while np.size(target_points > 0) and detected_spike: #while valid points and change has not stagnated
        detected_spike = False
        for index in range(np.size(target_points)):
            n = target_points[index]
            p = np.argmax(abs(drdx[n-1::-1]) < edge_slope) #find first point satisfying condition
            if p == 0 and drdx[n-1] >= edge_slope: #argmax returns 0 if can't find, checking if drdx[n-1-0] satisfies or argmax didn't find
                p = n
            p = p + 1
            x1 = dist[n-p]

            q = np.argmax(abs(drdx[n+1:-1]) < edge_slope) #find first point satisfying condition
            if q == 0 and drdx[n+1] >= edge_slope: #argmax returns 0 if can't find, checking if drdx[n+1+0] satisfies or argmax didn't find
                q = rows-n
            q = q + 1
            x2 = dist[n+q]
            ...
            if height > (w ** 2)/peak_factor:
                ...
                drdx[slice_low:slice_high] = update_drdx(rough, slice_low, slice_high, samp_dist)
                drdx[0] = drdx[1]
                drdx[-1] = drdx[-2]
                d2rdx2[slice_low:slice_high] = update_d2rdx2(rough, slice_low, slice_high, samp_dist)
...
def gen_drdx(rough, rows, samp_dist):
    ...
    drdx[start:stop] = (rough[start+1:stop+1] - rough[start-1:stop-1])/(2.0*samp_dist)
    drdx[0] = drdx[1]
    drdx[-1] = drdx[-2]
...
def gen_d2rdx2(rough, rows, samp_dist):
    ...
    d2rdx2[start:stop] = (rough[start-1:stop-1] - 2*rough[start:stop] + rough[start+1:stop+1])/(samp_dist ** 2)
...
def update_drdx(rough, slice_low, slice_high, samp_dist):
    ...
    sub_drdx = (rough[slice_low+1:slice_high+1]-rough[slice_low-1:slice_high-1])/(2.0*samp_dist)
    return sub_drdx
...
def update_d2rdx2(rough, slice_low, slice_high, samp_dist):
    ...
    sub_d2rdx2 = (rough[slice_low-1:slice_high-1] - 2*rough[slice_low:slice_high] + rough[slice_low+1:slice_high+1])/(samp_dist ** 2)
    return sub_d2rdx2
